...
test("Q2: why is that smaller than the bookings CareYield generated?", async ({ page }) => {
  await page.goto("/console/results");
  await expect(page.getByRole("heading", { name: "Why the smaller number is the real one" })).toBeVisible();
  await expect(page.getByTestId("results-split-total")).toContainText(/booked from a CareYield message/i);
...
test("the v1 detailed view is still reachable and unchanged", async ({ page }) => {
  await page.goto("/console/results");
  await page.getByRole("link", { name: "Detailed measurement view" }).click();
