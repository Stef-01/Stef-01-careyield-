...
| Voice care-finder (`app/page.tsx` → `app/care-finder.tsx`) | founder commits 603219f, e083d7a, ee627b6 | 2, 3, **G6, G7** | **MAPPED AS PROTOTYPE — not cleared for patient traffic.** Controls that DO apply today: synthetic clinicians only (no real practitioner is represented); WCAG AA enforced by the W49 sweep, which now covers this route; no ratings, stars or testimonials anywhere in the flow. Controls that DO NOT yet exist and are required before any patient-reachable deployment: no compliance linter covers this copy (W6 gates message templates, W23 the B2B bundle — neither reaches here); no G6 Ahpra advertising review of profile copy; no G7 ruling on matching that keys off a patient's stated care needs. `qa/audit-matching-trust/audit.md` records seven P1 honesty defects in the flow (fabricated match reasons, invented travel times, hidden voice substitution, unprovenanced capability claims), all open. Original exposure analysis: Patient-facing: free-text/voice description of a desired GP → matched named clinician with photo and appointment times. Three exposures: patient states a clinical need ("a GP who understands ADHD") and is matched on it (G7/TGA — matching must key on clinician attributes, not patient symptoms); named clinicians + photos in a patient-facing directory (G6 + Ahpra advertising: credential claims must be accurate and supportable, no testimonials); the eight portraits in `public/clinicians/` are not verified real practitioners |
