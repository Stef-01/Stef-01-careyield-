...
| Build status page (`app/console/founder`) | W310 (`src/founder/outstanding.ts`) | 3 | Behind sign-in and addressed to the founder. Every figure is derived from `BUILD-STATE.md` and §4 of the plan at render time, so the page cannot state a status the tree disagrees with. It names no service and makes no offer; its own sentences are the lint-gated `FOUNDER_COPY` bundle and everything else it renders is quotation from the plan and the ledger. No practice, patient or clinical content reaches it — W271 gives it a route class whose allowance excludes every module holding tenant state |
...
| Response graph (`app/console/responses`) | W220 (`src/outcomes/response-console.ts`) | 2, 3 | What was recorded after each thing Meherr did, shown to the practice it is about and to nobody else. **Proposed gate G9 is unratified, so there is no recipient, no delivery and no send control**, and the page states the absence rather than leaving it to read as a feature nobody built. The surface exists to keep FOUR silences apart, because it can show all four at once and merging any two is the failure: a kind never performed (positive knowledge about the product, **not a rate of zero**), a kind performed with nothing recorded against it (**a statement about the record, never about the people it concerns** — W211), a cell withheld because it describes too few people (measured, W218), and no graph at all — where the page genuinely cannot tell a quiet period from an unrecorded one and resolves to `cannot_determine` with what would settle it, because W179's rule is that the reassuring reading requires proof. Counts come from W218's disclosable form rather than the raw graph, so a cell below the floor is withheld before it reaches the page, and a single withheld cell forces a second so the first cannot be recovered by subtraction. Holds no patient identity — every number is an aggregate over intervention and response kinds, and a test scans a graph built over a real run for surviving record keys |
| Capacity console (`app/console/capacity`) | W229 (`src/capacity/console.ts`) | 2, 3 | What a practice's own diary recorded: how many of the slots it opened were left open at the end of the week, per clinician and per day of the week. **The surface exists to keep TWO emptinesses apart, and they lead a practice in opposite directions**: a session that filled every slot it offered (a FINDING — the record was read and it says the diary is full) and a session the record cannot answer for (no finding at all — nobody has written enough down). Rendered as the same blank cell a reader takes the first meaning, because it is the one needing no explanation, so each has its own state, its own sentence and its own count, and **there is no dash anywhere on the page** (W197's rule about a blank cell). The page-level blank says which blank it is in as many words — "the absence of a diary, not a diary with nothing left in it" — because a manager opening an empty capacity page and concluding the diary is full is the specific harm. **Rows are enumerated from the diary, never from the answers**: asking the forecaster what it can speak about makes the sessions it cannot speak about VANISH, and a vanished session is indistinguishable from one that does not exist. Every figure is a subtraction from W223's range rather than a second estimate, and W225's accuracy score travels with each answerable row. **No demand claim anywhere** — a fill rate measures uptake of what was offered, so a full session says nothing about anybody who was not offered a slot (W226's `no-demand-claim`, argued in the one place it is most tempting). No send control and no control at all; nothing here changes what Meherr sends. Holds no patient identity — a row is a clinician id, a weekday and three counts, and the type has no field a person could occupy |
| Interop console (`app/console/interop`) | W246 (`src/interop/console.ts`) | 2, 3 | What has left this practice for another system, and what happened to each. **The page a practice actually opens today is the EMPTY one, and rendering it as a table of zeros would be the defect**: a zero is a measurement, and this is the absence of one — "0 exchanges" reads as *we tried and nothing came of it*, or as *nothing needed to go*, and the truth is a third thing a reader will not supply. So the empty state says nothing was **attempted**, and lists what would have to be decided first, **derived from W242's credential register** rather than written into the page, with every gate on a path rather than the first (a list holding one blocker would show the path opening the day the other was ratified). **The page states a bound it cannot escape.** W239's ledger records what LEFT and W244 requires that an exchange which never left produce NO ROW — both right, and together they mean this console can never show a failure to send. That is rendered where the rows are, not in a footnote, because a reader who believes the page is complete reads no rows as no problems: *"An empty list means nothing left; it does not mean nothing failed."* **"Left, nothing came back" is neither a delivery nor a failure**, gets its own visibly different treatment, and is the only row that needs a person — W244's four states, with no `unknown` and no delivered/failed boolean anywhere on the surface. All four counts show every time including the zeros (W228), and there is no dash (W197). **No control at all, and here that is load-bearing**: a retry button would be the most dangerous control in this product, because one click on an unanswered row may put a second copy of a clinical document into another practice's records — the retry verdict is SHOWN so a person can act, and there is nothing to press. No success rate, since every rate would be over the survivors of a filter the page cannot see. Holds no patient identity — a ledger row deliberately carries none, and a console that joined the subjects back in would recreate the lasting copy the ledger was shaped to avoid |
| Platform API (`app/api/v1/[endpoint]`) | W253 (`src/api/surface.ts`) | 2, 3 | A practice reading its own data over HTTP, read-only. **The practice cannot be named from outside**: a reader is handed a `ReadContext` whose only field is the practice id and which contains no request — no `Request`, no `searchParams`, no header, no body — so `?practiceId=prac-2` is not a parameter to remember to ignore, it is a parameter with nowhere to be read. Y4-1's fix (`complaintsFor(practiceId)`, filtered as the query) is right for a page, which gets its id from the session, and **is not enough for an endpoint, which is asked for one by a stranger**: every correctly scoped read in this tree would answer such a request correctly and disclosingly. **One door.** The whole API is dispatched by a single route file that resolves the practice once, from the session, through `activePracticeFor` — where membership is the grant and the practice cookie is only a preference that cannot widen access (W166). A test counts the route files under the API root and fails on a second, so an unscoped endpoint is not something to catch in review; there is nowhere to put one. **Read-only structurally**: `ApiEndpoint` has a `read` and no other verb, the route exports `GET` and a test scans for the other four. **The envelope stamps the practice** (W227: an answer that does not say what it rests on), because a payload gets cached, forwarded and pasted into a ticket. Holds no patient identity — every endpoint returns practice-level aggregates or the practice's own projected record, built field by field rather than spread, so a field added to `PracticeRecord` cannot be published by a commit that does not touch this surface |
...
/console/founder — Build status page
/console/interop — Interop console
...
/console/responses — Response graph
/console/capacity — Capacity console
