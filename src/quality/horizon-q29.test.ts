// W364 verify gate: "the rule's preconditions evaluated and RECORDED in this plan before any unit
...
// is not.
//
// AND THIS ONE PRICES A MOMENT WITH NOTHING OPEN IN IT, which is new. The last two expansions each
// had a sibling session holding a row and had to name it so the figures would not move the day it
// landed. Q28 finished before this ran: every row W352–W363 is `done` and the only claim in the
// ledger is this unit's own. `IN_FLIGHT_AT_EXPANSION` is therefore EMPTY — and it is still here,
// still exported and still driven, because an exemption list that disappears when it is empty is
// one nobody re-reads when it stops being.
//
// THE EMPTINESS IS ASSERTED RATHER THAN ASSUMED, which is this quarter's own theme arriving in the
// test that opens it: a list is a population, and an empty population that nothing distinguishes
// from a broken derivation is exactly what W369 is for.
...
const RAW = readFileSync(path.join(ROOT, "docs/HORIZON-Q29.md"), "utf8");
...
export const Q29_HORIZON_LAST_UNIT = 364;

/**
 * Rows this document did NOT price as done, by name.
 *
 * EMPTY, AND THE EMPTINESS IS A FACT ABOUT THE MOMENT rather than a field nobody filled. Q28 closed
 * before this expansion ran, so no sibling session held a row while the figures were taken. Kept as
 * an exported list because the last two expansions each needed one, and a mechanism that vanishes
 * between the occasions it is needed is one somebody re-invents badly.
 */
export const IN_FLIGHT_AT_EXPANSION: readonly string[] = [];
...
const asAtHorizon = () => rows().filter((r) => r.n <= Q29_HORIZON_LAST_UNIT);
const Q29 = Array.from({ length: 13 }, (_, i) => `W${365 + i}`);
...
const doneAsAt = (): Row[] =>
  asAtHorizon().filter(
    (r) =>
      ...
      (r.status === "done" || r.n === Q29_HORIZON_LAST_UNIT),
  );

/** §5n alone, ending at whatever heading comes next — W312's rule about a slice with a shelf life. */
const section = () => {
  const start = PLAN.indexOf("## 5n.");
...
describe("W364 the document reads the ledger it claims to read", () => {
  ...
  it("holds an empty in-flight list, and can tell that from a broken one", () => {
    // THE ARM THIS QUARTER IS ABOUT. An empty exemption list and an exemption list nothing populates
    // read identically in a done count, so the emptiness is checked against the ledger rather than
    // taken on trust: no row other than this unit's own is claimed, which is WHY it is empty.
    // Driven to non-empty first, on the same producer: an empty list nobody has seen fill is a
    // check nobody has seen work, which is W293's rule and this quarter's theme in miniature.
    ...
    expect(without.length, "naming a row changes nothing, so the exemption is decoration").toBe(
      doneAsAt().length - 1,
    );
  });

  it("states the ledger size before and after this expansion", () => {
    expect(asAtHorizon()).toHaveLength(Q29_HORIZON_LAST_UNIT);
    expect(DOC).toContain(`${Q29_HORIZON_LAST_UNIT} week-units`);
    expect(DOC).toContain(`${Q29_HORIZON_LAST_UNIT + Q29.length} after it`);
    ...
    expect(
      rows().filter((r) => r.n <= Q29_HORIZON_LAST_UNIT + Q29.length),
      "the expansion did not lay down its rows",
    ).toHaveLength(Q29_HORIZON_LAST_UNIT + Q29.length);
  });
...
describe("W364 the six preconditions are evaluated against the thing each claims", () => {
  it("(1) expands one quarter of thirteen, and no theme beyond it", () => {
    ...
    expect(PLAN).toContain("## 5n. Year 7 — Q29 (W365–W377)");
    expect(section(), "§5n plans a unit beyond the quarter it expands").not.toContain("W378");
  });

  it("(2) cites the two documents the rule names, by path", () => {
    for (const cited of ["docs/AUDIT-Y5.md", "docs/GATE-DOSSIER-Y5.md", "src/quality/quarter-mutants-q27.ts"]) {
  ...
  it("(4) adds no blocked row, and the count has not moved", () => {
    ...
    expect(plannedBlocked(Q29), "Q29 planned a blocked row").toEqual([]);
    expect(DOC).toContain("Q29 adds no blocked row");
...
describe("W364 the quarter table describes the units that were laid down", () => {
  ...
  it("ends the quarter on a gate and a close, and holds a hardening pass", () => {
    ...
    expect(notes.get("W376")).toMatch(/gate/i);
    expect(notes.get("W377")).toContain("QUARTER CLOSE");
...
describe("W364 the theme is derived from evidence that exists", () => {
  it("names the units the theme is read from, and the ledger holds each as done", () => {
    const byId = new Map(rows().map((r) => [r.id, r]));
    for (const id of ["W353", "W355", "W358", "W359", "W360", "W361", "W362"]) {
  ...
  it("quotes its figures as history, and says which day each was true of", () => {
    ...
    for (const figure of ["295 modules", "sixty-four", "thirty gaps", "fifty-nine sites"]) {
      expect(DOC, `the theme quotes ${figure} and it is not in the document`).toContain(figure);
    }
    expect(DOC).toContain("as at this expansion");
    // And the document does not claim any of them is live.
    expect(DOC).not.toContain("295 modules today");
  });

  it("does not price a row somebody else was holding, because there was not one", () => {
    // The two quarters before this each had to name a sibling session's row. This one says why it
    // does not: the quarter closed first. Asserted against the ledger so the sentence cannot outlive
  ...
  it("refuses to set a numeric gate, and says why", () => {
    ...
    expect(DOC).toContain("Four quarters have now refused a number");
  });

  it("states the gate this quarter will be re-read against", () => {
    expect(DOC).toContain("every population this document names is derived");
    expect(DOC).toContain("W376 re-reads the list");
  });
});
