...
export const NAMED_CONDITIONS: readonly NamedCondition[] = [
  ...
  },
  {
    ...
    reading: {
      kind: "owed",
      by: "W384",
      why: "The gap is real and the tree already holds one instance of it: `/console/referrals` renders its day-two emptiness as a LIST, so the page's most important zero is not in this population and is covered only because W346's register holds the route. What would read it is a derivation of what a COMPONENT RENDERS rather than of what an expression is called. RE-AIMED THREE TIMES, AND THIS ONE IS DIFFERENT IN KIND. W361 owed it to W363, which re-read which way each named check fails; W364 re-pointed it at W371 on the grounds that a link graph was what this condition wanted, and it was not — W371 derives which routes are REACHABLE, and reachability is not what a page renders its zero AS. Both were guesses at which unit would happen to want the work. W377 is the quarter close, whose job is to say what the next quarter CARRIES, so rather than guess a fourth time it wrote the derivation into the plan: W384 takes a page\'s zero states from what the component renders instead of from the expressions it calls, and this condition is its stated subject rather than a hope about its scope. W370 IS WHY THIS WAS CAUGHT BEFORE THE COMMIT INSTEAD OF AFTER: the clock is a close-gate reader now, and closing W377 with the promise still aimed at it fails `verify:close` rather than `main`."
    },
  },
  {
