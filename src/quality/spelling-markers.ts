...
export const MARKERS: readonly Marker[] = [
  ...
  },
  {
    ...
    matches: "a line beginning `export interface` or `export type` followed by a name",
    standing: {
      kind: "blind",
      looksLike:
        "`export type { Probe } from \"./x\";` — a type RE-EXPORT, which is ordinary TypeScript and is a type by every definition, and which reaches this classification as nothing because the name it exports sits inside braces.",
      plausibility: "idiomatic",
      probe: (root) =>
        twoSpellings(
          root,
          ...
          (copy) => typeNames(readFileSync(path.join(copy, PROBE_FILE), "utf8")).includes("Probe"),
        ),
    },
  },
  {
