...
export function acceptanceCarryingModules(root: string): string[] {
  ...
  for (const file of sourceModules(root)) {
    ...
    // the scan to code rather than reword a comment that is saying the right thing (W198).
...
export const ACCEPTANCE_BOUND =
  "The date check is real for all seven and is the thing this unit fixed: every acceptance in the tree is now compared against a clock, in one place, and a register added tomorrow is caught by the census before anybody has to remember. The RE-DERIVATION is not uniform and is not claimed to be. Two registers are re-swept here and a stale acceptance in either fails this suite. Four are cited to an assertion inside their own test file, and a resolved citation says the check is written, not that it is right — W284's central citation resolved to `text.includes(\"/\")` and read as coverage for a quarter. Moving those four comparisons out of their test files is W289's remedy and a different unit's work. One is `by_review` and can never be re-swept: nothing re-reads a diff, which is why its date is the whole of its liveness.";
