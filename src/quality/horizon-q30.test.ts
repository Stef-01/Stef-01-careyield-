// W377 verify gate: "the rule's preconditions evaluated and RECORDED in this plan before any unit
...
// is not.
//
// AND THIS ONE PRICES A MOMENT WITH A ROW OPEN IN IT. W374 was held by the other builder while the
// figures were taken, so `IN_FLIGHT_AT_EXPANSION` names it and the done count excludes it. The last
// expansion needed no exemption and kept the mechanism anyway, on the argument that a list which
// disappears between the occasions it is needed is one somebody re-invents badly. This is the
// occasion, and the list is driven both ways rather than trusted.
//
...
const RAW = readFileSync(path.join(ROOT, "docs/HORIZON-Q30.md"), "utf8");
...
export const Q30_HORIZON_LAST_UNIT = 377;

/**
 * Rows this document did NOT price as done, by name.
 *
 * W374, held by the other builder while the figures were taken. It is neither done nor abandoned,
 * and pricing it as either would make the done count a guess about somebody else's session — so it
 * is named, excluded, and the document says so in the same words. The list was empty at the last
 * expansion and kept anyway; this is the occasion it was kept for.
 */
export const IN_FLIGHT_AT_EXPANSION: readonly string[] = ["W374"];
...
const asAtHorizon = () => rows().filter((r) => r.n <= Q30_HORIZON_LAST_UNIT);
const Q30 = Array.from({ length: 13 }, (_, i) => `W${378 + i}`);
...
describe("W377 the document reads the ledger it claims to read", () => {
  ...
  it("names the row that was in flight, and can tell that from naming nothing", () => {
    ...
    expect(IN_FLIGHT_AT_EXPANSION, "the expansion priced a moment with nothing open in it").toEqual(["W374"]);
  ...
  it("states the ledger size before and after this expansion", () => {
    expect(asAtHorizon()).toHaveLength(Q30_HORIZON_LAST_UNIT);
    expect(DOC).toContain(`${Q30_HORIZON_LAST_UNIT} week-units`);
    expect(DOC).toContain(`${Q30_HORIZON_LAST_UNIT + Q30.length} after it`);
    ...
    expect(
      rows().filter((r) => r.n <= Q30_HORIZON_LAST_UNIT + Q30.length),
      "the expansion did not lay down its rows",
    ).toHaveLength(Q30_HORIZON_LAST_UNIT + Q30.length);
  });
...
describe("W377 the six preconditions are evaluated against the thing each claims", () => {
  it("(1) expands one quarter of thirteen, and no theme beyond it", () => {
    ...
    expect(PLAN).toContain("## 5o. Year 7 — Q30 (W378–W390)");
    expect(section(), "§5o plans a unit beyond the quarter it expands").not.toContain("W391");
  });

  it("(2) cites the two documents the rule names, by path", () => {
    for (const cited of ["docs/AUDIT-Y5.md", "docs/GATE-DOSSIER-Y5.md", "src/quality/hardening-q28.ts"]) {
  ...
  it("(4) adds no blocked row, and the count has not moved", () => {
    ...
    expect(plannedBlocked(Q30), "Q30 planned a blocked row").toEqual([]);
    expect(DOC).toContain("Q30 adds no blocked row");
...
describe("W377 the theme is derived from evidence that exists", () => {
  it("names the units the theme is read from, and the ledger holds each as done", () => {
    const byId = new Map(rows().map((r) => [r.id, r]));
    for (const id of ["W326", "W363", "W367", "W370", "W371", "W375", "W376"]) {
  ...
  it("quotes its figures as history, and says which day each was true of", () => {
    ...
    for (const figure of ["fifty files", "twice on `main` in one day", "two gigabytes"]) {
    ...
    expect(DOC).not.toContain("fifty files today");
  });
  ...
  it("refuses to set a numeric gate, and says why", () => {
    ...
    expect(DOC).toContain("Five quarters have now refused a number");
  });

  it("states the gate this quarter will be re-read against", () => {
    expect(DOC).toContain("every moment this document names is derived");
    expect(DOC).toContain("W389 re-reads the list");
  });
});
