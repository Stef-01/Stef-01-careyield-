...
export const FINDINGS: readonly HardeningFinding[] = [
  ...
  },
  {
    ...
    disposition: {
      ...
      by: "W327",
      evidence:
        "W327 scoped the walk to the shared exclusion list and moved `statSync` inside the `try`. BOTH CONSEQUENCES WERE LIVE, and the second was worse than the finding said: `SELF_REFERENCE_BOUND` opens exactly while fewer than two fixture files exist, so a dependency shipping ONE lifted a stated bound and W297 would have reported it stale — a red suite caused by a lockfile. Driven as a pair rather than as a fix: a `.fixtures` planted under `node_modules` must not move the sweep or the bound, and one planted under `src/` must move both, so a walk narrowed until it sees nothing fails beside a walk that reads the install. The broken symlink is driven too, on a dangling link in a copied tree. AND THE UNDERLYING SHAPE IS FIXED RATHER THAN THE INSTANCE: this walk had no exclusion list, `tree-walks.ts` had six entries and `register-census.ts` had three, so three recursions over the same tree meant three different trees. `EXCLUDED_DIRECTORIES` is now one exported set and all three import it.",
    },
  },
  {
    id: "Q24-CR-8",
