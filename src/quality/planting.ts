...
export const WRITES_WITHOUT_A_PLANTER: Readonly<Record<string, string>> = {
  ...
  "src/quality/hook-reach.test.ts":
    "Writes a global-setup file, a test file and a vitest config into a directory of its own and then RUNS them as a child process, to measure which of the two run-level hooks a killed run reaches. `withPlantedIn` cannot do it: it plants into a copy of this repository, whose config is this repository's — the whole reading is of a run wired to a different one. The child is killed by design, so this test's own `finally` is what removes the directory, and the name carries `probeDirPrefix` so W375's sweep reclaims it when the `finally` is the thing that gets skipped.",
  "src/quality/shared-state.test.ts":
    "Writes planted test files and a vitest config into a directory of its own under `tmpdir()` and runs a REAL child suite over them, because what one test file can see of another is a fact about the pool this project is configured with rather than something a table can assert. `withPlantedIn` plants into a copy of this repository, where the files would be read as text by the registers and never executed; here they have to be executed, so the plant is a runnable project outside the tree. The directory is removed in a `finally`, and its name carries `process.pid` so W375's sweep owns it.",
  "src/quality/import-cycles.test.ts":
    "Writes a two-module `.mjs` pair into a directory of its own under `tmpdir()` and imports it, to watch Node evaluate a real cycle rather than to put a file in front of a scan. `withPlantedIn` cannot do it: it plants into a COPY OF THIS REPOSITORY, where a `.ts` file is only ever read as text by a scanner — the whole point here is that the runtime loads the pair, so the plant has to be a file the loader itself will accept, outside the tree and named `.mjs`. The removal is this test's own `rmSync` in a `finally`, per pair rather than per assertion, and the directory name carries `process.pid` so W375's sweep can tell a leak of this run's from another run's.",
