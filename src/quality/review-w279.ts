...
export const FINDINGS: readonly HardeningFinding[] = [
  ...
  },
  {
    ...
    disposition: {
      ...
      by: "W334",
      evidence:
        "`readInterestSignups` returns a result that distinguishes the three zeros W279 named: a MISSING file is nobody having signed up, an EMPTY one is the same, and a file holding a line that will not parse is a truncated append or an unmounted volume — where the count shown is lower than the count held and nothing said so. `/console/interest` renders the third as `could_not_load` and says in those words that the figure is short. The route's declaration in `CONSOLE_ZERO_STATES` moved with it, and W279's refusal was right to wait: the state is declared now because the page can reach it, which is the order this tree insists on. Driven on a constructed store: missing, empty and truncated files read as three different things, and the rows the truncated file DID hold still come back.",
    },
  },
  {
    id: "W279-SIMP-1",
