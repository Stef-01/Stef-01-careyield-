...
export const FINDINGS: readonly HardeningFinding[] = [
  ...
  },
  {
    ...
    disposition: {
      kind: "deferred",
      why: "The remedy is a store that distinguishes 'no file', 'rows dropped' and 'no signups', plus a page branch to render it — product work, and W287 is a review unit. Deferring is the honest call rather than declaring a state the page cannot show. The remedy is written into `FALLIBLE_READS` so the next unit inherits it as a sentence rather than rediscovering the finding. W318 RETARGETED THIS FROM `W288+`, which it had pointed at for thirty-one units: a range names no unit, so no unit is ever the one it was deferred to and nothing could ever report it unanswered. W325 is the quarter close, and this register now fails if that lands with this still deferred. W325 READ IT AND RETARGETED IT TO W334, WHICH IS THE SECOND RETARGET AND IS ITSELF THE SIGNAL. W334 walks a practice whose setup is INCOMPLETE and requires every unfinished step named where an operator would look for it, which is this finding stated as product work: a store that returns `[]` for 'no file', 'rows dropped' and 'no signups' alike cannot tell an operator which of the three they are looking at, and that is W179's rule at the page rather than a register's. The remedy is unchanged and is written in `FALLIBLE_READS`; what changed is that it now points at a unit whose own gate cannot be met while this is true, rather than at a unit that would merely be nearby.",
      by: "W334",
    },
  },
  {
