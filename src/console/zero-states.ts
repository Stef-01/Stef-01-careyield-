...
export const CONSOLE_ZERO_STATES: readonly RouteZeroStates[] = [
  ...
  { route: "/console/interest", states: ["nothing_yet", "could_not_load"], why: "W287 CORRECTED THIS ARGUMENT. It read 'signups arrive from the public form, so empty means nobody has signed up' — a sentence about where the DATA comes from, when the zero is about where the READ goes. This is the ONE console route whose read reaches disk: `.data/interest-signups.jsonl`. W334 DECLARED `could_not_load` HERE AT LAST, and the order matters: W279 refused to declare it while `listInterestSignups` returned `[]` for a missing file, an unparseable one and an empty one alike, because a control declared where the page cannot reach it is the paper trail of a control that does not exist. `readInterestSignups` now distinguishes them — a missing file is nobody having signed up, and a file holding a line that will not parse is a truncated append or an unmounted volume, where the count shown is lower than the count held. The page renders the second as `could_not_load` and says so in that many words." },
...
export const RUNTIME_BOUND =
  "This proves the vocabulary and the classification, not the rendering. It does not prove a page shows the RIGHT state when a read fails — that needs a read that can be made to fail on demand. W287 CORRECTED THE CLAIM THAT USED TO STAND HERE: this said every console read is an in-memory store call that cannot throw, and that is true of all but one of them. `/console/interest` reads a JSONL file on disk, so `could_not_load` IS reachable on one route — it is still declared on none, because the store cannot tell the page which zero it is holding, and W287's `FALLIBLE_READS` carries the route and the remedy. It does not prove a page has a branch to put a declared state in either: a source detector for that was written, measured, and found to disagree with the hand classification in BOTH directions, because this tree writes emptiness as `length === 0`, as a ternary over an array, as a `??`, and as a delegated component. What IS proved: the three vocabularies differ pairwise on every field, every console route is classified with an argument each, and the route list is checked against W271's register in both directions. The remedy for both gaps is the same — a store that can be made to fail and a rendered page to read — and it arrives with W275's gate and a real database rather than with a sharper regex.";
