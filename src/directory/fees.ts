...
export const RATING_RULE_OVER_BROAD =
  "W23's `no-ratings` matches the bare word \"review\", which has now produced false positives on three surfaces: /clinicians (the identifier `is-reviewed`, W192), /privacy/automated-decisions (an ADM notice is required to say how a decision can be reviewed, W192, accepted), and fee service names (\"Care plan review\", W198, exempted here). A patient rating is caught separately by the star, 5/5 and \"rated\" branches of the same rule. Narrowing the word branch is a unit of its own because W23's landing tests depend on the current pattern.";
