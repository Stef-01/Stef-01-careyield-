...
const NAMESPACES: Record<string, () => Promise<Record<string, unknown>>> = {
  ...
  "src/interop/disclosure-ledger.ts": () =>
    import("@/interop/disclosure-ledger"),
  ...
  "src/interop/consent-to-disclose.ts": () =>
    import("@/interop/consent-to-disclose"),
  ...
  "src/compliance/public-surfaces.ts": () =>
    import("@/compliance/public-surfaces"),
  ...
  "src/quality/number-words.ts": () => import("@/quality/number-words"),
  "src/quality/runtime-population.ts": () =>
    import("@/quality/runtime-population"),
  "src/quality/patterns.ts": () => import("@/quality/patterns"),
  ...
  "src/quality/mutation-sampling.ts": () =>
    import("@/quality/mutation-sampling"),
  ...
  "src/quality/failure-direction.ts": () =>
    import("@/quality/failure-direction"),
  "src/quality/flattering-numbers.ts": () =>
    import("@/quality/flattering-numbers"),
  "src/quality/quarter-mutants-q28.ts": () =>
    import("@/quality/quarter-mutants-q28"),
  "src/quality/quarter-mutants-q29.ts": () =>
    import("@/quality/quarter-mutants-q29"),
  "src/quality/close-sensitivity.ts": () =>
    import("@/quality/close-sensitivity"),
  "src/quality/welded-comparisons.ts": () =>
    import("@/quality/welded-comparisons"),
  "src/quality/derivable-lists.ts": () => import("@/quality/derivable-lists"),
  "src/quality/hardening-q28.ts": () => import("@/quality/hardening-q28"),
  "src/quality/hardening-q29.ts": () => import("@/quality/hardening-q29"),
  ...
  "src/quality/assertion-vocabulary.ts": () =>
    import("@/quality/assertion-vocabulary"),
  ...
  "src/quality/founder-page-facts.ts": () =>
    import("@/quality/founder-page-facts"),
  "src/console/waiting.ts": () => import("@/console/waiting"),
  "src/quality/quarter-mutants-q26.ts": () =>
    import("@/quality/quarter-mutants-q26"),
  "src/quality/unapplied-remedies.ts": () =>
    import("@/quality/unapplied-remedies"),
  ...
  "src/quality/defaulted-registers.ts": () =>
    import("@/quality/defaulted-registers"),
  "src/quality/quarter-mutants-q27.ts": () =>
    import("@/quality/quarter-mutants-q27"),
  "src/quality/horizon-directions.ts": () =>
    import("@/quality/horizon-directions"),
  ...
  "src/quality/name-conventions.ts": () => import("@/quality/name-conventions"),
  ...
  "src/quality/decision-moments.ts": () => import("@/quality/decision-moments"),
  "src/quality/shared-state.ts": () => import("@/quality/shared-state"),
  ...
  "src/quality/order-independence.ts": () =>
    import("@/quality/order-independence"),
  "src/quality/order-regressions.ts": () =>
    import("@/quality/order-regressions"),
  ...
  "src/outcomes/response-console.ts": () =>
    import("@/outcomes/response-console"),
  "src/outcomes/response.ts": () => import("@/outcomes/response"),
  "src/tenancy/fixture-coherence.ts": () =>
    import("@/tenancy/fixture-coherence"),
...
describe("W200 the four rail properties plus the fifth, re-derived", () => {
  it("carries all five, each with a Y4 re-derivation and a test that enforces it", () => {
    ...
    for (const property of RAIL_PROPERTIES) {
      expect(
        property.statement.length,
        `${property.id} states nothing`,
      ).toBeGreaterThan(80);
      expect(
        property.establishedBy.length,
        `${property.id} cites no unit`,
      ).toBeGreaterThan(0);
      // The gate's word is RE-DERIVED. A one-line "still true" is the thing it forbids.
      expect(
        property.y4Rederivation.length,
        `${property.id} is asserted, not re-derived`,
      ).toBeGreaterThan(200);
      expect(property.enforcedBy, `${property.id} names no test`).toMatch(
        /\.test\.ts/,
      );
    }
  });
  ...
  it("names a Y4 unit in every re-derivation, so none of them is about the old tree", () => {
    for (const property of RAIL_PROPERTIES) {
      const units = [...property.y4Rederivation.matchAll(/\bW(\d+)\b/g)].map(
        (m) => Number(m[1]),
      );
      expect(
        units.length,
        `${property.id} cites no unit at all`,
      ).toBeGreaterThan(0);
      expect(
...
describe("W200 the declared copy surface is checked against the tree", () => {
  it("declares every Y4 module, and declares no module that is not one", () => {
    ...
    expect(OPERATOR_COPY_SURFACES.map((s) => s.module).sort()).toEqual(
      y4Modules(),
    );
  ...
  it("says why the rest of a module is not operator copy, especially when nothing is", () => {
    for (const surface of OPERATOR_COPY_SURFACES) {
      expect(
        surface.notCopy.length,
        `${surface.module} declares no reason`,
      ).toBeGreaterThan(60);
    }
  });
  ...
  it("names only exports that exist, and only exports with text in them", async () => {
    ...
    for (const surface of OPERATOR_COPY_SURFACES) {
      ...
      for (const exportName of surface.operatorCopy) {
        expect(
          namespace,
          `${surface.module} has no export ${exportName}`,
  ...
  it("reaches copy outside src/education/, which is the gap this unit found", () => {
    ...
    for (const declared of EDUCATION_COPY_MODULES) {
      expect(OPERATOR_COPY_SURFACES.map((s) => s.module)).not.toContain(
        declared,
      );
    }
    const linted = OPERATOR_COPY_SURFACES.filter(
      (s) => s.operatorCopy.length > 0,
    );
    expect(
      linted.length,
      "nothing outside education is actually linted",
    ).toBeGreaterThan(10);
  });
});
...
describe("W200 the product informs and never advises, across everything Y4 added", () => {
  it("finds no unaccepted advice in any operator copy Y4 added", async () => {
    ...
    expect(
      unaccepted.map(
        (f) => `${f.module}#${f.exportName}: ${f.rule} on "${f.match}"`,
      ),
    ).toEqual([]);
  });
  ...
  it("still fires on advice, so the clean result means something", () => {
    ...
    expect(
      lintEducationCopy("Consider a follow-up for this patient.").map(
        (v) => v.rule,
      ),
    ).toContain("no-soft-recommendation");
    expect(
      lintEducationCopy("This patient needs a review.").map((v) => v.rule),
  ...
  it("accepts by module, export, rule and matched string, and every acceptance is live", async () => {
    ...
    for (const accepted of ACCEPTED_COPY_FINDINGS) {
      ...
      expect(
        accepted.why.length,
        `${accepted.module} is accepted without an argument`,
      ).toBeGreaterThan(150);
    ...
    expect(
      ACCEPTED_COPY_FINDINGS.length,
      "an acceptance list nobody had to write",
    ).toBeGreaterThan(0);
  });
