...
export const OPERATOR_COPY_SURFACES: readonly CopySurface[] = [
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W394's record of which registers rest on a naming habit and what a file spelling it otherwise would cost. Nobody but a developer reads it. Its longest strings are each row's cost and `CONVENTION_BOUND`; it constructs no record of any kind, and no patient, condition or appointment reason appears in any of it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W388's register is reviewer-facing: file names, test titles and sentences about what a citation points at. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/decision-moments.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W387's register is reviewer-facing: function names, parameter names and sentences about when a decision is taken. It says nothing to a patient and nothing to an operator, and no clinical judgement is made or described — the rules it reads are the product's, and it reads their SIGNATURES.",
  },
  {
    module: "src/quality/shared-state.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W385's register is reviewer-facing: file names, call names and sentences about what a test run shares. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/hook-reach.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W382's register is reviewer-facing: file names, hook names and sentences about when a test run ends. Nothing here renders and no practice sees it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W375's register is reviewer-facing: file names, function names and sentences about temporary directories. Nothing here renders and no practice sees it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W393's register is reviewer-facing: module paths, number-words, and sentences about what a comment says. It renders nothing and no practice sees it. The prose it quotes is this repository's own commentary rather than words anybody is shown, which is the distinction W373's row draws about the copy it resolves.",
  },
  {
    module: "src/quality/patterns.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W391's register is reviewer-facing: module paths, constant names, regex literals and sentences about what each one enumerates. It renders nothing and no practice sees it. The quoted patterns are source text rather than words anybody is shown, which is the same distinction W373's row draws about the copy it resolves.",
  },
  {
    module: "src/quality/reached-pages.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W371's register is reviewer-facing: route paths, file names and arguments about navigation. It renders nothing and no practice sees it.",
  },
  {
    module: "src/quality/empty-populations.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W369's register is reviewer-facing: register names and sentences quoted out of this tree's own doc comments, several of which quote a founder gate in order to record that it is closed. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/subject-and-walk.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W367's register is reviewer-facing: bound names, walk names and sentences quoted out of this tree's own bounds. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/populations.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W365's register is reviewer-facing: module paths and the name of the walk each one calls. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/horizon-directions.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W363's register is reviewer-facing: tokens quoted from a planning document, the modules they resolve to, and the citation or argument standing behind each. Nothing here renders and no practice sees it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W355's register is reviewer-facing: module paths, function names, argument positions and the files that drive each one. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/console/zero-meaning.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W361's register is reviewer-facing: route paths, the expressions pages interpolate, and the argument for what a zero at each would mean. The operator copy it is ABOUT lives on the pages; nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/spec-stores.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W359's register is reviewer-facing: spec paths, store module paths and the argument for each gap the suite keeps. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/spec-premises.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W358's register is reviewer-facing: spec paths and the argument for whether each one reads its premise back. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/unapplied-remedies.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W357's register is reviewer-facing: mutant ids, module paths, and the argument for whether each recorded remedy was built. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/quarter-mutants-q26.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W349's register is reviewer-facing: module paths, mutation-operator ids, lines of this repository's own source quoted so a reader can see what changed, and the argument for each survivor and each excluded module. Nothing here renders and no practice sees it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W347's register is reviewer-facing: module paths, exported function names, and the argument for where each fact reaches the founder page or why it does not. The page's own words live in `FOUNDER_COPY` and are linted there; nothing in this file renders.",
  },
  {
    module: "src/quality/unasked-facts.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W340's register is reviewer-facing: module paths, exported function names, and the sentence naming the screen that would ask for each fact. Those sentences DESCRIBE console screens and none of them is one — nothing here renders, and the screens they name do not exist, which is the register's whole point.",
  },
  {
    module: "src/quality/escape-hatches.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W345's re-reading is reviewer-facing: module paths, exported constant names, and the argument for each declaration that a check cannot be made to fail. Nothing here renders and no practice sees it.",
  },
  {
    module: "src/quality/controls.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W337's gate is reviewer-facing: unit ids, module paths, exported names, and the argument for each control the horizon plans. The longest strings on it are QUOTATIONS from a planning document, carried so an argument can be checked against the sentence it rests on. Nothing here renders.",
  },
  {
    module: "src/quality/unrun.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W333's register is reviewer-facing: module paths, branch ids, and the argument for each thing the unit suite cannot reach. Nothing here renders and no practice sees it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W328's register is reviewer-facing: a path this repository has no reason to hold and a paragraph saying what its presence would be evidence of. `uncleanMessage` composes a sentence, but the reader is whoever is looking at a failed test run, not a practice — and nothing here renders.",
  },
  {
    module: "src/quality/claim-classes.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W324's gate is reviewer-facing: unit ids, module paths, exported names, and the argument for each class the horizon plans. The longest strings on it are QUOTATIONS from a planning document, carried so an argument can be checked against the sentence it rests on. Nothing here renders and no practice sees it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W352's register of which way each register in W267's census fails — loud, or toward looking correct. Nobody but a developer reads it. Its longest strings are the argument for each open row and `DIRECTION_BOUND`; its inputs are the census's own entries, and no patient, condition or appointment reason appears in any of it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W386's record of the mutation sweep over Q29's modules — the population, what it could not reach, and what it closed. Nobody but a developer reads it. Its longest strings are each row's argument and `Q29_MUTANT_BOUND`; it constructs no record of any kind, and no patient, condition or appointment reason appears in any of it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W366's register of which text-scanning markers a second spelling of their own subject gets past. Nobody but a developer reads it. Its longest strings are the argument for each marker and `SPELLING_BOUND`; the values it constructs are short TypeScript source fragments planted into a copied tree — a fold, a path literal, a `reviewBy` field — and no patient, condition or appointment reason appears in any of it.",
  },
  {
    module: "src/quality/shared-excuses.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W356's register of the reason-strings more than one register entry stands behind, and whether anything can contradict them. Nobody but a developer reads it. Its longest strings are quoted REASONS from other registers — a store's scoping argument, a bound's escape hatch — reproduced so a row can be matched to the sentence it reads, and `EXCUSE_BOUND`. It constructs no record of any kind: every value it handles is source text from this repository.",
  },
  {
    module: "src/quality/superset.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W353's register of what each population-taking derivation returns when handed an input it cannot understand. Nobody but a developer reads it. Its longest strings are the argument for each selector and `SUPERSET_BOUND`; its inputs are this tree's own exported functions, called against this repository, and no patient, condition or appointment reason appears in any of it.",
  },
  {
    module: "src/quality/horizon-claims.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W350's gate for Q27: every claim the horizon document emphasises, classified as read by a named check, unread with its reason, or a statement about the plan rather than the tree. Nobody but a developer reads it — the HORIZON is founder-facing and lives in `docs/`, and this module holds only the classification. Its longest strings are the quoted claims, the argument for each and `HORIZON_CLAIM_BOUND`; its inputs are that document and this tree's own check names, and no patient, condition or appointment reason appears in any of it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W343's hardening pass over Q26 — four findings, each with the unit whose diff introduced it and a disposition carrying a clock. Nobody but a developer reads it. Its longest strings are the finding texts, their evidence and `Q26_HARDENING_BOUND`; its inputs are this repository's own diffs and registers, and no patient, condition or appointment reason appears in any of it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W341's register of the modules holding their own copy of a parse this tree shares. Nobody but a developer reads it. Its longest strings are the cost of each shared parse, the standing argued for each site and `PRIVATE_COPY_BOUND`; its inputs are the text of this repository's own modules and short planted snippets of TypeScript, and no patient, condition or appointment reason appears in any of it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W329's standings: where the unit a disposition cites stands in the ledger, and which of those standings can never answer it. Nobody but a developer reads it. Its longest strings are the argument for each standing and `DEFERRAL_BOUND`; its inputs are the ledger and this tree's own hardening findings, and no patient, condition or appointment reason appears in any of it.",
  },
  {
  ...
  },
  {
    ...
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W326's closing gate: the ledger-reading checks, run against the row as it will be committed. Nobody but a developer reads it. Its longest strings are the argument for each watched check, the argument for each excused module and `CLOSE_GATE_BOUND`; its inputs are this tree's own ledger, plan and source files, and no patient, condition or appointment reason appears in any of it.",
  },
  {
    module: "src/quality/closing-state.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W315's closing state: the ledger row a unit will commit, run through the checks that read a row before the commit exists. Nobody but a developer reads it. Its longest strings are the arguments for each check and `CLOSING_BOUND`; its inputs are the ledger, the plan and the tree's own stated bounds, and no patient, condition or appointment reason appears in any of it.",
  },
  {
    module: "src/quality/hardening-q24.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W311's record of Q24's three review lenses: eleven findings, each with the unit that introduced it, a disposition and a date. Nobody but a developer reads it. Its longest strings are the findings themselves, which describe this tree's own registers and the seams its consolidations left; its inputs are diffs and source files, and no patient, condition or appointment reason appears in any of it.",
  },
  {
    module: "src/founder/outstanding.ts",
    operatorCopy: ["FOUNDER_COPY"],
    notCopy:
      "The rest is a derivation over `BUILD-STATE.md` and §4 of the plan: parses, diffs and a wait figure. `FOUNDER_BOUND` is a sentence about what the derivation cannot judge, addressed to a developer. Everything the page RENDERS other than `FOUNDER_COPY` is quotation — §4's own gate sentences and the ledger's own notes — so the words this module chooses are exactly the ones declared above, and no patient, condition or appointment reason appears in any of it.",
  },
  {
    module: "src/quality/manifest.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W305's one row per module: the census entry, the blind spot and the refusal branches each module owes, declared once and derived into the three registers that used to hold them separately. Nobody but a developer reads it. Its longest strings are the census prose and the stated bounds it now carries on each module's behalf, all of them about this tree's own detectors; its inputs are the tree's own source files, and no patient, condition or appointment reason appears in it.",
  },
  {
    module: "src/quality/register-counts.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W304's sweep for assertions pinning a declared register's SIZE to a literal, and the ratchet mechanism for any that must stay. Nobody but a developer reads it. Its longest strings are the argument for telling a register size apart from a fixture count and `COUNT_BOUND`; its inputs are the text of the tree's own test files, and no patient, condition or appointment reason appears in it.",
  },
  {
    module: "src/quality/planting.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W303's planting harness: the scoped planters every register uses to put a file in front of a detector, the directories a copied tree needs, and the suites that write an artefact rather than a probe. Nobody but a developer reads it. Its longest strings are the argument for scoping a plant and the reasons attached to each declared writer; its inputs are file paths and source text, and no patient, condition or appointment reason appears in it.",
  },
  {
    module: "src/quality/citations.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W301's citation resolver: the `<file> :: <assertion>` format, the three distinct reasons a citation fails to resolve, and the two files that split the separator for a composite id rather than a citation. Nobody but a developer reads it. Its longest strings are the argument for keeping the three failure causes apart and `CITATION_BOUND`; its inputs are file paths and source text, and no patient, condition or appointment reason appears in it.",
  },
  {
    module: "src/quality/hardening-q23.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W298's Q23 hardening record: the units read, the two not read and why, the units the reviewer wrote themselves, and four findings from three lenses with their dispositions. Nobody but a developer reads it. Its longest strings are the findings — a lifting predicate with no root parameter, eleven pinned counts, a citation format implemented five times, and the security evidence for a quarter of dev-only registers — and no patient, condition or appointment reason appears in any of them.",
  },
  {
    module: "src/quality/mutation-sampling.ts",
    operatorCopy: [],
    notCopy:
      "Deliberately empty. W296's mutation sampler: five character-level operators, the sites they apply to, the modules with no suite of their own, and the three sampled changes no suite noticed. Nobody but a developer reads it. Its longest strings are the remedies attached to those three survivors — a single-clinician roster, a search filter passed as a search term, and a second close/rejoin cycle — and the argument for each operator; no patient, condition or appointment reason appears in any of them, and its only input is the text of the tree's own modules.",
  },
  {
