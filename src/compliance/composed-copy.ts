...
export const COMPOSED_COPY_SITES: readonly ComposedSite[] = [
  ...
  { module: "src/quality/shared-excuses.ts", fn: "withheldClassesThatArePersisted", composes: "W356's contradiction message: the module whose withholding sentence has stopped being true, and the handling W106 gives it instead. Developer-facing and read only from a failing assertion — it names a file path and a classification word, and no patient, condition or appointment reason can reach it." },
  { module: "src/quality/quarter-mutants.ts", fn: "quarterModules", composes: "The refusal W353 added: the range it was handed, when its ends are not numbers. Developer-facing and a throw rather than a report — the only sentence it composes is the message of a mistake at a boundary, and it names two numbers and nothing else." },
  ...
  { module: "src/quality/self-ending.ts", fn: "eventInWords", composes: "The event a declaration waits for, put into a reader's words for the sentence a failed build prints — `W334 landing`, `G5 being ruled on`, `a remedy for the ledger`. Developer-facing, and phrased per kind rather than as one opaque string, because a reader meeting a wait wants to know which unit or which gate, not that a predicate went true." },
  { module: "src/quality/repository-clean.ts", fn: "uncleanMessage", composes: "One sentence naming every artefact the run left in the repository, each with what its presence is evidence of. Developer-facing, read by whoever is looking at a failed run: it names the path and then says what mechanism leaves that shadow, because a bare path tells a reader nothing about what to go and look for." },
  ...
  { module: "src/quality/unread-bounds.ts", fn: "staleOwedConditions", composes: "One line per bound whose `owed` reading names a unit that has landed, giving the bound and the unit. Developer-facing throughout — it is read by W326's close gate SUITE, by W339's arm over the live tree, and by this module's suite, never rendered — and it is here for the reason W278 exists: the detector FINDS composing functions rather than being told about them, so a register that exempted the ones nobody renders would be the hand-kept list this one replaced." },
