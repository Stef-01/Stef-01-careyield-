...
export const FIXTURE_BOUND =
  "Five of the eighteen are driven with real inputs. The other thirteen take assembled reports, response graphs and audit exports, and constructing one by hand would produce a fixture that tests this author's idea of an `AgreementReport` rather than the one the tree builds — which is the fabricated-fixture failure W234 recorded. Every one of the eighteen is still covered by the literal sweep, which is the check with teeth; what the thirteen lack is the interpolation half. The remedy is to reach the modules' own test fixtures rather than to invent new ones, and that is a different unit's work.";
